# Retry data preparation with fixes:
# - Use df.info() directly (no custom buffer)
# - Use caas_jupyter_tools.display_dataframe_to_user for key tables
# - Provide clearer printed outputs
import pandas as pd
import numpy as np
from sklearn.preprocessing import StandardScaler
import caas_jupyter_tools as ctools

file_path = "/mnt/data/train(2).csv"
df = pd.read_csv(file_path)

print("=== Original data sample (first 5 rows) ===")
ctools.display_dataframe_to_user("original_head", df.head())

print("\n=== Info ===")
df.info()

print("\n=== Basic statistics for numeric columns ===")
ctools.display_dataframe_to_user("original_describe", df.describe().T)
...
ctools.display_dataframe_to_user("missing_summary", missing_df.head(30))
...
out_path = "/mnt/data/processed_train.csv"
...
ctools.display_dataframe_to_user("processed_head", processed.head())
...
ctools.display_dataframe_to_user("processed_columns", pd.DataFrame({"columns": processed_X.columns.tolist()[:200]}))
