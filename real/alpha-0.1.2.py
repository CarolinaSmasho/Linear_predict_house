...
def main():
    ...
    inp_data = input("(B)uy or (S)ell or (0)Exit: ").strip().upper()
    ...
    while inp_data != "0":
        if inp_data == "S":
            ...
            # Get LotArea and SalePrice input with validation
            while True:
                try:
                    inp = input(f"Selected Neighborhood: {selected_neighborhood}\nEnter LotArea, SalePrice (space-separated): ").split()
                    if len(inp) != 2:
                        print("Error: Please enter exactly two values (LotArea and SalePrice).")
                        input("Press Enter to continue...")
                        continue
                    lot_area = int(inp[0])
                    sale_price = int(inp[1])
                    if lot_area <= 0 or sale_price <= 0:
                        print("Error: LotArea and SalePrice must be positive numbers.")
                        input("Press Enter to continue...")
                        continue
                    break
                except ValueError:
                    print("Error: Please enter valid integers for LotArea and SalePrice.")
                    input("Press Enter to continue...")
            ...
            # Create input vector as a DataFrame to match feature names
            input_vector = pd.DataFrame(
                [[1, input_data["LotArea"], input_data["SalePrice"]]],
                columns=["Neighborhood_match", "LotArea", "SalePrice"]
            )
            ...
            print("Invalid input. Please enter 'B', 'S', or '0'.")
        
        inp_data = input("\n(B)uy or (S)ell or (0)Exit: ").strip().upper()
